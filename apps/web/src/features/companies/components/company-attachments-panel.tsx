...
import { useCompanyAttachments } from "../hooks/use-company-attachments";
...
export interface CompanyAttachmentsPanelProps {
  companyId: string;
}

export function CompanyAttachmentsPanel({ companyId }: CompanyAttachmentsPanelProps): React.ReactElement {
  const { attachments, isLoading, uploadAttachment, isUploading, deleteAttachment } =
    useCompanyAttachments(companyId);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Paperclip} title="No attachments" description="Upload files related to this company." />
      ) : (
        <div className="flex flex-col gap-2">
          {attachments.map((attachment) => (
            <div key={attachment.id} className="flex items-center gap-3 rounded-md border border-border bg-card p-3">
